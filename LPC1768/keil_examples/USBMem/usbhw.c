...
#pragma arm section zidata = "USB_RAM"      /* data located in USB RAM (neccessry for DMA) */
                                            /* see file DMA.sct */
uint32_t UDCA[USB_EP_NUM];                  /* USB Device communication Area first in USB RAM */
uint32_t DD_NISO_Mem[4*DD_NISO_CNT];        /* Non-Iso DMA Descriptor Memory (4 DWORDs each) */
uint32_t DD_ISO_Mem [5*DD_ISO_CNT];         /*     Iso DMA Descriptor Memory (5 DWORDs each) */
...
#else
  LPC_USB->EpIntEn   = 0xFFFFFFFF;   /* ignore USB_DMA_EP if USB_DMA not configured */
#endif
...
uint32_t USB_ReadEP (uint32_t EPNum, uint8_t *pData) {
  ...
  if (USB_DMA_IsUsed (EPNum)) {      /* Check if EP is used for DMA */
    ...
    return (cnt);                    /* data is already copied to BulkBuf */
  }
#endif
...
uint32_t USB_WriteEP (uint32_t EPNum, uint8_t *pData, uint32_t cnt) {
  ...
  if (USB_DMA_IsUsed (EPNum)) {      /* Check if EP is used for DMA */
    DD.BufAdr  = (uint32_t)pData;    /* DMA Buffer Address */
    DD.BufLen  = cnt;                /* DMA Buffer Length in bytes */
    DD.MaxSize = 64;                 /* Maximum packet size	*/
...
void USB_DMA_ReqSet (uint32_t EPNum) {
  LPC_USB->DMARSet = 1<<EPAdr(EPNum);
}
...
uint32_t USB_DMA_IsUsed (uint32_t EPNum) {
  uint32_t EPPhyAdr;

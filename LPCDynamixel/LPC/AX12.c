...
#define sendData(args)  (sendchar(args))    // Write Over Serial
#define availableData() (UARTAvailable())    // Check Serial Data Available
#define readData()      (getkey())         // Read Serial Data
#define peekData()      (UARTPeek())         // Peek Serial Data


// Macro for Timing

//#define delay32Us(1,args) (delayMicroseconds(args))  // Delay Microseconds

// Macro for Comunication Flow Control

//#define setDPin(DirPin,Mode)   (pinMode(DirPin,Mode))       // Select the Switch to TX/RX Mode Pin
//#define switchCom(Mode)        LPC_GPIO1->DATA |= (1<<8);       // Switch to TX/RX Mode (Changed to function)
...
void switchCom(Mode)
{
	if (Mode == Tx_MODE) LPC_GPIO1->DATA |= (1<<8); //TX
	else LPC_GPIO1->DATA &= ~(1<<8);                //RX

}

int AX12_read_error(void)
{
	Time_Counter = 0;
...
int AX12_setEndless(unsigned char ID, bool Status)
{
 if ( Status ) {	
...
int AX12_turn(unsigned char ID, bool SIDE, int Speed)
{		
		if (SIDE == 0){                          // Move Left///////////////////////////
...
int AX12_torqueStatus( unsigned char ID, bool Status)
{
...
int AX12_ledStatus(unsigned char ID, bool Status)
{    
